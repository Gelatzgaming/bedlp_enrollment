...
//         header('location: ../edit.teacheristrar.php?teacher_id=' . $teacher_id);
//     } else {
//         $image = addslashes(file_get_contents($_FILES['prof_img']['tmp_name']));
//         $set_userInfo = mysqli_query($conn, "UPDATE tbl_teacheristrars SET img = '$image' WHERE teacher_id = '$teacher_id'") or die(mysqli_error($conn));
...
//             $insertUser = mysqli_query($conn, "UPDATE tbl_teacheristrars SET teacher_fname = '$firstname', teacher_lname = '$lastname', teacher_mname = '$midname', email = '$email', username = '$username', password = '$hashpwd' WHERE teacher_id = '$teacher_id'") or die(mysqli_error($conn));
...
if (isset($_POST['submit']) && $_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if ($failed != 0) {
        ...
        $hashpwd = password_hash($password, PASSWORD_BCRYPT);
        $insertUser = mysqli_query($conn, "UPDATE tbl_teachers SET img = '$image', teacher_fname = '$teacher_fname', teacher_lname = '$teacher_lname', teacher_mname = '$teacher_mname', email = '$email', username = '$username', password = '$hashpwd' WHERE teacher_id = '$teacher_id'") or die(mysqli_error($conn));
