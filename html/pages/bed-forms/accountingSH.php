...
while ($row = mysqli_fetch_array($get_stud)) {
    ...
    $pdf->Image('../../assets/images/auth/logo.jpg', 25, 9, 15, 15);
    $pdf->Image('../../assets/images/auth/logo1.jpg', 125, 9, 15, 15);
    ...
    $pdf->Cell(155, 5, '045 Admiral Village, Talon 3, Las ' . utf8_decode('Piñas') . ' City, 1740', 0, 1, 'C');
    ...
    $pdf->Rect(135, 26.5, 25, 6);
    $pdf->Cell(155, 1, 'PRINCIPAL\'S COPY', 0, 1, 'R');
    ...
    $pdf->Cell(40, 3, '', 0, 0);
    $pdf->Cell(20, 2, $row['lrn'], 0, 1);

    $pdf->Ln(1);
    $pdf->Cell(10, 7, 'SY:', 0, 0);
    $pdf->Cell(20, 7, $row['academic_year'], 0, 0, 'C');
    $pdf->Cell(1, 7, '', 0, 0);

    $pdf->Cell(22, 7, 'Student Type:', 0, 0);
    $pdf->SetFont('Arial', '', 9);
    $pdf->Cell(19, 7, $row['stud_type'], 0, 0, 'C');
    $pdf->Cell(5, 7, '', 0, 0);

    $pdf->Cell(15, 7, 'Grade:', 0, 0);
    $pdf->Cell(15, 7, $row['grade_level'], 0, 0, 'C');
    $pdf->Cell(6, 7, '', 0, 0);

    $pdf->Cell(20, 7, 'Section/Strand: ', 0, 0);
    $pdf->Cell(5, 7, '', 0, 0);
    $fontsize = 7;
    ...
    if (!empty($row['section'])) {
        ...
        $pdf->Cell(15, 7, 'TBA - ' . $row['strand_name'], 0, 0, 'C');
    }
    $pdf->Cell(20, 5.5, '', 0, 1);
    $pdf->SetFont('Arial', '', 9);
    $pdf->Ln(1);
...
while ($row = mysqli_fetch_array($get_stud)) {
    ...
    $pdf->Ln(1);
    $pdf->SetFont('Arial', 'B', 7);
    $pdf->Rect(132, 26.5, 28, 6);
    $pdf->Cell(155, 1, 'ACCOUNTING\'S COPY', 0, 1, 'R');
...
while ($row = mysqli_fetch_array($get_stud)) {
    ...
    $pdf->SetFont('Arial', 'B', 7);
    $pdf->Rect(135, 26.5, 25, 6);
    $pdf->Cell(155, 1, 'REGISTRAR\'S COPY', 0, 1, 'R');
...
while ($row = mysqli_fetch_array($get_stud)) {
    ...
    $pdf->SetFont('Arial', 'B', 7);
    $pdf->Rect(135, 26.5, 25, 6);
    $pdf->Cell(155, 1, 'STUDENT\'S COPY', 0, 1, 'R');
